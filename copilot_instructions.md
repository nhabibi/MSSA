...
Build a reproducible Python pipeline for HIV-1 RNA coevolution analysis consistent with the proposal (TAR preliminary study, AIM1 full genome modeling, AIM2 comparative analysis).

GENERAL RULES
- Input: raw HIV-1 nucleotide FASTA sequences (LANL database)
- Subtypes: strictly split into B and O (never mix)
- Reference coordinate system: HXB2 (K03455)
- MAFFT for alignment, MESSI for inference (CLI calls)
- All steps logged and outputs saved per phase
